  ...
  try {
    ...
    if (checkIfUserExists) {
      ...
      try {
        ...
        res.status(201).json({
          message:
            "New user account has been created. Login to find other climbers nearby.",
          user: {
...
const login = async (req, res) => {
  try {
    ...
    res.status(400).json({
      message: "Server error, login failed: cannot check if user exists.",
      error: error,
    });
    console.log("error", error, res);
  }
...
const updateProfile = async (req, res) => {
  try {
    ...
    res.status(400).json({
      message: "Server error, updating user data failed. Please try again.",
      error: error,
    });
    console.log("error", error, res);
  }
...
const deleteProfile = async (req, res) => {
  try {
    ...
    res.status(400).json({
      message: "Server error, deleting user profile failed",
      error: error,
    });
    console.log("error", error, res);
  }
